...
import { mean } from '../signal-processing';
import type { PPGPreprocessingConfig, AccelPreprocessingConfig, QualityScoreConfig } from '../signal-processing/types';
import { DEFAULT_PPG_CONFIG, DEFAULT_ACCEL_CONFIG, DEFAULT_QUALITY_CONFIG } from '../signal-processing/types';
import { PPGConfig, AccelConfig } from '../constants/sensor';

import type { BiosignalFeatures, RRResult, ExtractionFailureReason, TimeDomainFeatures, NonLinearFeatures, AccelFeatures } from './types';
import { getFailureDescription } from './types';
import { decodePPGAsDouble, decodeAccel, decodeTemp } from './binary-decoder';
import { preprocessPPG, isValidPPGForProcessing } from './ppg-preprocessing';
import { extractRRIntervalsWithDiagnostics } from './rr-extraction';
 ...
 * @param tempData Raw temperature binary data
 ...
 * @param temp Temperature value (single reading)
...
export function extractWithDiagnostics(
  ...
  const tempMean: number | null = hasTemp ? mean(tempSamples) : null;

  // 5. Compute signal quality score
...
function buildFeatures(
  ...
  return {
    ...
    // Temperature (null if temp missing)
    tempMean,
