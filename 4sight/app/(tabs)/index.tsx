...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchRiskPrediction, 60 * 1000);
  ...
  if (!prediction) {
    return (
      <View style={[styles.container, styles.centered]}>
        <Text style={styles.errorText}>Unable to load prediction data</Text>
      </View>
    );
  }

  // Use the timeToRiskRange for the danger window
  const { lower, upper } = prediction.timeToRiskRange;

  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Current Risk Trends</Text>
        <Text style={styles.subtitle}>Based on your biometric data</Text>
        
        <View style={styles.alertBadge}>
          <Text style={[
            styles.alertText,
            prediction.alertLevel === 'high' && styles.alertHigh,
            prediction.alertLevel === 'medium' && styles.alertMedium,
          ]}>
        ...
        <View style={styles.statsContainer}>
          <View style={styles.statBox}>
            <Text style={styles.statLabel}>Risk Susceptibility</Text>
            <Text style={styles.statValue}>
              {(prediction.overallSusceptibility * 100).toFixed(1)}%
            </Text>
          </View>
          <View style={styles.statBox}>
            <Text style={styles.statLabel}>Confidence</Text>
            <Text style={styles.statValue}>
              {(prediction.modelConfidence.average * 100).toFixed(0)}%
            </Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.infoCard}>
        <Text style={styles.infoTitle}>Risk Window Details</Text>
        <Text style={styles.infoText}>
          Peak risk expected in approximately {Math.round(prediction.timeToRiskMinutes)} minutes
        </Text>
        <Text style={styles.infoSubtext}>
          Confidence interval: {prediction.timeToRiskRange.confidenceInterval}
        </Text>
      </View>
    </ScrollView>
  );
